...
- **Top strip.** A 48px row without a divider: the sidebar toggle stays fixed at the top left and its chevron shows the action — pointing left to collapse while the sidebar is open, right to expand while it is closed. Search appears next to it when the sidebar is hidden so it remains available on a board; the save status sits at the far right in 12px `--text-3`.
- **Search.** A centred modal takes a text query and shows project, note, and image matches from every project. Each result names its project; selecting an item opens it in focus on its project page. Keep the result surface quiet and readable, with an image thumbnail where available. Escape closes search. ⌘K/Ctrl+K opens it from anywhere.
...
- The library drawer is a full-height 320px panel on the right, like an inspector: it mirrors the sidebar (`--bg-sidebar`, a `--line-1` edge, a 48px header row, the search field at 56px) and makes room by narrowing the board rather than covering it. Below 700px it lies over the board instead. It opens and closes like the sidebar: it slides in over 180ms while the board narrows in step. “Add from library” is its toggle and stays where it is; while the panel is open it becomes a pressed quiet button, “Hide library”, and the panel's own ✕ closes it too. Drag an item onto the board or press “+” to use the next free space. Images open at their aspect ratio; when a card is resized away from it, the image stays whole and the spare space is the card's own surface, not a separate band. Notes can be written directly on the board.
