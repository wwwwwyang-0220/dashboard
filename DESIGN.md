...
- **Project page.** Content is a column at most 1232px wide (1296px with its 32px gutters), centred in the space beside the sidebar, so opening or closing the sidebar shifts the whole page together and any spare width is split evenly on both sides. The heading uses the same column, and so does a board: its heading and cards share the centred column. Two columns 32px apart: the library fills the main column; a 300px rail on the right holds the to-dos, then the boards, 40px apart, and stays in view while the library scrolls. Below 1000px the rail moves above the library, with to-dos and boards side by side where they fit.
