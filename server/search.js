...
export function searchProjects(projects, query, limit = 60) {
  ...
  for (const project of projects) {
    ...
    for (const item of project.items ?? []) {
      ...
      const contentMatch = item.type === 'note' && terms.every((term) => searchable(`${item.title} ${item.content}`).includes(term))
      ...
      matches.push({
        ...
        title,
        snippet: item.type === 'note' ? excerpt(item.content ?? '', terms) : '',
